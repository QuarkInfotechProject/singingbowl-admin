...
const Reviewspage = () => {
  ...
  const getMenus = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/orderSupport`, {
        method: "GET",
        headers: {
  ...
  console.log("support dataasdfdsf", supportData);
  const toggleCreateDialogs = (id: number) => {
    setSelectedItemId(id);
    setIsSheetOpen(true);
    //  router.push(`/admin/order-support/show/${id}`)
  };
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Order support </CardTitle>
        <CardDescription>Details of order support</CardDescription>
      </CardHeader>
                  ...
                  <TableCell colSpan={5} className="h-48">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="text-lg font-semibold text-gray-900 mb-2">
                        No Order Support Found
                      </h3>
                      <p className="text-gray-500 text-center">
                        There are currently no order support to display.
                      </p>
                    </div>
                  </TableCell>
                </TableRow>

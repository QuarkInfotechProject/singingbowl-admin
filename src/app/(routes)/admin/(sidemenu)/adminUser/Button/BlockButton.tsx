...
const BlockButton: React.FC<BlockButtonProps> = ({
  ...
  const handleBlockClick = () => {
    setBlockEndUserDTO({ ...blockEndUserDTO, uuid: userUuid });
  };
  ...
  const handleSaveChangesClick = async () => {
    try {
      ...
      const updatedBlockEndUserDTO = { ...blockEndUserDTO, remarks };
      ...
      if (response.data.code === 0) {
        ...
        onBlockSucess();
          ...
          toast({
            description: response?.data?.message || 'Super Admin Cannot  block ',
            variant: 'destructive',
          });
      ...
      <Dialog>
        ...
        <DialogContent className="sm:max-w-[525px]">
          ...
          <DialogFooter className="mt-4 items-center w-full">
            ...
            <DialogClose asChild>
                <Button  className="cursor-pointer rounded-lg w-full bg-red-500 hover:bg-red-600">
                  Cancel
                </Button>
              </DialogClose>
              <Button

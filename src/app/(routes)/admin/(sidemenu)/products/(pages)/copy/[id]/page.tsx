...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  // Check if token or id doesn't exist
  if (!token || !id) {
    return null;
  }
  let data = null;
  ...
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/products/show/${id}`, {
      headers: {
    ...
    return <RootLayout defaultData={data} defaultProduct={defaultProduct} uuid={id} />;

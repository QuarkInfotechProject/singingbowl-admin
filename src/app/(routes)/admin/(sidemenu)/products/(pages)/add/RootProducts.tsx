...
const RootProducts = ({
  ...
  const handleRemove = (
    ...
    if (toRemove === "related-product") {
      const updatedIds = relatedProducts.filter((idx:any) => idx !== id);
      form.setValue("relatedProducts", updatedIds);
    } else if (toRemove === "cross-sell-product") {
      const updatedIds = crossSells.filter((idx:any) => idx !== id);
      form.setValue("crossSells", updatedIds);
    } else if (toRemove === "up-sell-product") {
      const updatedIds = upSells.filter((idx:any) => idx !== id);
      form.setValue("upSells", updatedIds);
    } else {
      console.log("invalid to delete");
      return;
    }
  };

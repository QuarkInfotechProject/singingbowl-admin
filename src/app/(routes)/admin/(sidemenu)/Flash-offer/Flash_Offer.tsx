...
const Flash_Offer = () => {
  ...
  const onDelete = async (id: string) => {
    ...
    try {
      ...
      if (res?.status === 200) {
        ...
        toast({
          description: "Failed to delete the flash offer",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const getOrder = async () => {
    try {
      ...
      const res = await clientSideFetch({
        url: `/flash-offers?page=${currentPage}`,
        method: "get",
        toast: "skip",
      });
      if (res?.data) {
  ...
  const onToggleSwitch = async (id: number) => {
    ...
    try {
      const res = await clientSideFetch({
        url: "/flash-offers/change-status",
        method: "post",
        toast: "skip",
        body: { id },
      });

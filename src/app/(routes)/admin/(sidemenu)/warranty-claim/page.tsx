...
import { WarrantyClaimsResponse } from "@/app/_types/warranty-claim/warraty";
...
const Warrantypage = () => {
  ...
  const getMenus = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`/api/warranty-claim?page=${currentPage}`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    if (isFiltering === false) {
      getMenus().then(() => {
        // Set filtering to false after fetch completes
      });
    }

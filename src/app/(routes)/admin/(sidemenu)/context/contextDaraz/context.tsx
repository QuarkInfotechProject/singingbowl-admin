...
// Define action types
type ProcessAction = 
  | { type: 'SET_DARAZ_DATA'; payload: TransactionsResponse }
  | { type: 'SET_CURRENT_PAGE'; payload: number }
  | { type: 'SET_TOTAL_PAGES'; payload: number }
  | { type: 'TOGGLE_SHEET' }
  | { type: 'FILTER_Acitivity'; payload: { transactionCode?: string; paymentMethod?: string; } };
...
// Create the reducer
const orderReducer = (state: ProcessState, action: ProcessAction): ProcessState => {
  switch (action.type) {
    case 'SET_DARAZ_DATA':
...
export const fetchDaraz= async (
    dispatch: React.Dispatch<ProcessAction>, 
    currentPage: number, 
    filters: { transactionCode?: string; paymentMethod?: string; } = {}
  ) => {
        try {
      const url = `/api/darazCount?page=${currentPage}`; 
      ...
      dispatch({ type: 'SET_DARAZ_DATA', payload: data });

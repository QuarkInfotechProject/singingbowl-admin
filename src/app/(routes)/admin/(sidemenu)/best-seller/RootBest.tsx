...
const RootNewLaunches = () => {
  ...
  const onDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/bestSeller/del`, {
        method: "POST",
  ...
  const getOrder = async () => {
    try {
      setLoading(true);
      const url = `/api/bestSeller?page=${currentPage}`;
  ...
  const onToggleSwitch = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/bestSeller/status`, {
        method: "POST",
  ...
  console.log("selecediten", selectedItemId);
  return (
              ...
              <TableBody>
                {orderData?.data.map((item: LaunchData) => (
                  ...
                  >
                        ...
                        .map((file: File, index: number) => (
                          <div key={index} className="mb-2">
                            ...
                            file.imageUrl.endsWith(".jpeg") ? (
                              ...
                              <p>Unsupported file type: {file.imageUrl}</p>
                            )}
                          </div>
                        ))}
                    </TableCell>
                    <TableCell>
                      {item.files
                        .filter((file: File) => file.zone === "mobileFile")
                        .map((file: File, index: number) => (
                          <div key={index} className="mb-2">
                            {file.imageUrl.endsWith(".png") ||
                            file.imageUrl.endsWith(".jpg") ||
                  ...
                  className={`inline-block px-2 py-1 rounded-full text-xs font-semibold ${
                    item.isActive === 1 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
                  }`}
                >

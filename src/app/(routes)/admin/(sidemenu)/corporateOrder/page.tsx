...
import RootCorporate from "./RootCorporate";
...
const CorporateOrderPage = () => {
  const [corporateData, setCorporateData] = useState<ApiResponse | null>(null);
  ...
  const getMenus = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/corporateOrder`, {
        method: "GET",
        headers: {
  ...
  return (
    <div>
      ...
      <div className="flex justify-end items-center mt-6 select-none">
        ...
        >
          <div
            className={`relative inline-flex items-center rounded-l-md cursor-pointer opx-2 py-2 text-gray-700  hover:text-black focus:z-20 focus:outline-offset-0 ${

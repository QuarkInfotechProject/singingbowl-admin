...
function FileUploader({
  ...
  const [filesToUpload, setFilesToUpload] = useState<
    { file: File; categoryId: string; preview?: string }[]
  >([]);
  ...
  const LoadingSkeleton = () => (
    <div className="space-y-2">
      {[1, 2, 3].map((index) => (
        <div
          key={index}
          className="flex items-center gap-2 rounded-lg overflow-hidden border border-border p-2"
        >
    ...
    <div>
      ...
      <div className="max-h-[60vh] overflow-auto">
        {isLoading || isFetching ? (
          <div className="mt-6">
            <p className="font-medium mb-2 text-muted-foreground text-sm">
              Uploading files...
            </p>
            <LoadingSkeleton />
          </div>
      ...
      {filesToUpload.length > 0 && !isLoading && !isFetching && (
        <div className="mt-4 flex justify-end">
          <Button
            onClick={handleSubmit}
            disabled={filesToUpload.length === 0 || isLoading || isFetching}
